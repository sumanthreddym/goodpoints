 ...
 */
async function getLoyaltyRewardInformation(loyaltyAccountId) {
...
async function adjustLoyaltyPoints(accountId, points, reason) {
  

  const { result: { event } } = await loyaltyApi.adjustLoyaltyPoints({
    "adjust_points": {
      "points": points,
      "reason": reason
    }
